...
from file import File, ContentType
...
class Writer:
    ...
    def save_translated_book(self, file: File, output_file_path: str = None, file_format: str = "PDF"):
        if file_format.lower() == "pdf":
            self._save_translated_book_pdf(file, output_file_path)
        elif file_format.lower() == "markdown":
            self._save_translated_book_markdown(file, output_file_path)
        else:
            raise ValueError(f"Unsupported file format: {file_format}")

    def _save_translated_book_pdf(self, file: File, output_file_path: str = None):
        if output_file_path is None:
            output_file_path = file.pdf_file_path.replace('.pdf', f'_translated.pdf')

        LOG.info(f"pdf_file_path: {file.pdf_file_path}")
    ...
    def _save_translated_book_markdown(self, file: File, output_file_path: str = None):
        if output_file_path is None:
            output_file_path = file.pdf_file_path.replace('.pdf', f'_translated.md')
